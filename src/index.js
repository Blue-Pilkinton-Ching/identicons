...
const log = new Logger(
  'identicons',
  (import.meta.url === import.meta.main && process.env.LOG_LEVEL) || 'fatal'
)
...
ellipse.ellipse(0.5, 0.5, 0.5, 0.5, 0, 0, Math.PI * 2)
...
function randomPointInEllipse(prng, ctx) {
  ...
  if (is_in_path) return [x, y]
  else return randomPointInEllipse(...arguments)
}
...
function renderDiscs(opts, canvas) {
  ...
  if (width === 0 || height === 0)
    throw new Error(
      `Bad width or height (x = ${width}px, y = ${height}px); Cannot paint on an invisible canvas`
    )

  log.debug('Render for size %s/%s with seed: %s', width, height, seed)
  const ratio = Math.min(width, height) / Math.max(width, height)
  const maxSize = opts.maxSize || Math.max(width, height) * (ratio + 0.5)
  const minSize = opts.minSize || Math.min(width, height) * 0.1
  ...
  log.debug(
    'BG constraints: %s-%s = %s = %s',
    bg_range_min.toFixed(2),
    bg_range_max.toFixed(2),
    hue_selection,
    bg_hue
  )
